...
module turnstile_base() {
  ...
  base_width = 20;
  base_height = 10;
  corner_radius = 4; // Rounding radius for the base corners
  axle_diameter = 8;
  axle_height = 30;
  c_hole = 0.2;
  ...
  difference() {
    union() {
      // Rounded base plate using hull of cylinders at corners
      translate([0, 0, base_height / 2])
        hull() {
          for (x = [-(base_length / 2 - corner_radius), base_length / 2 - corner_radius]) {
            for (y = [-(base_width / 2 - corner_radius), base_width / 2 - corner_radius]) {
              translate([x, y, 0])
                cylinder(h=base_height, r=corner_radius, center=true);
            }
          }
        }
  ...
  difference() {
    union() {
      ...
      translate([0, 0, hub_h - arm_height]) {
        for (r = [0, 90, 180, 270]) {
          rotate([0, 0, r]) {
            // Main arm body (shortened by arm_thickness/2 to accommodate rounded tip)
            translate([hub_d / 2 - 0.5, -arm_thickness / 2, 0])
              cube([arm_length - hub_d / 2 + 0.5 - arm_thickness / 2, arm_thickness, arm_height]);
            // Rounded tip
            translate([arm_length - arm_thickness / 2, 0, 0])
              cylinder(h=arm_height, d=arm_thickness);
          }
        }
      }
    }
